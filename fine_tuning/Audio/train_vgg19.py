...
def train_vgg19(x, y):
    input = Input(batch_shape=(None, 1, 96, 64,), name='input')
    model = VGG19(include_top=True, weights=None, input_tensor=input, input_shape=None, classes=1000)
...
resized_audio = Ah.get_resized_audio(input_audio, win_frames=23520, print_info=False)
acoustic_data = Ah.compute_STFT_and_MelBank(resized_audio, 512, 64, print_info=False)
acoustic_data = acoustic_data.reshape(acoustic_data.shape[0], 1, acoustic_data.shape[1], acoustic_data.shape[2])

print(acoustic_data.shape)

b = np.ones((584, 1000))
m = train_vgg19(acoustic_data, b)