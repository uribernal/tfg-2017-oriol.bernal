...
def generate_arrays_from_file(videos):
    for video in videos:
        l = np.array([])
        with h5py.File('/home/uribernal/Desktop/MediaEval2017/data/visual_data.h5', 'r') as hdf:
            data = np.array(hdf.get('features/'+video))
            labels = np.array(hdf.get('labels/'+video))
            data = data.reshape(data.shape[0], data.shape[1], data.shape[2]*data.shape[3]*data.shape[4])
            data = data[:, :120, :]
        for i in range(data.shape[1]):
            l = np.append(l, labels)
        l = l.reshape(int(l.shape[0]/(3*120)), 3, 120)
        l = l.transpose(0, 2, 1)
        l = l[:, :, :2]
...
def train_model(model, experiment_id, dropout_probability, batch_size):
    ...
    figures_path = '/home/uribernal/PycharmProjects/tfg-2017-oriol.bernal/results/figures/raw_video/'+\
                   '{min:05}_Raw_Video_e_{experiment_id}_b{batch_size:03}_d{drop_out:02}.png'

    # Path for the weights
    store_weights_file = 'Raw_Video_e_{experiment_id}_b{batch_size:03}_d{drop_out:02}.hdf5'
    ...
    history = model.fit(x_train,
                        ...
                        shuffle=True)
    '''
    history = model.fit_generator(generate_arrays_from_file(movies),
...
if __name__ == "__main__":
    ...
    for experiment_id in range(1, 30):
        ...
        description = 'Experiment {0}: Raw Video, Using callbacks, drop-out={1}, batch-size={2}.'.format(
            experiment_id, drop_out, batch_size)

        # Get the LSTM model
        model = Mg.lstm_raw_video(batch_size, True)
